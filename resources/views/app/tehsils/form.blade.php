...
@section('content')
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="card-title mb-0">{{ $title ?? ($isEdit ? 'Edit' : 'Create') . ' Tehsil' }}</h5>
            <a href="{{ route('app.tehsils.index') }}" class="btn btn-secondary">Back to List</a>
        </div>

        <div class="card-body">
            <form action="{{ $isEdit ? route('app.tehsils.update', encryptId($model->id)) : route('app.tehsils.store') }}" 
                  method="POST" id="tehsilForm">
                @csrf
                @if($isEdit)
                    @method('PUT')
                @endif
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="province_id" id="province_id" class="form-control select2" required>
                            ...
                            @foreach($provinces as $id => $title)
                                <option value="{{ $id }}" {{ old('province_id', $isEdit && $model->district ? $model->district->province_id : '') == $id ? 'selected' : '' }}>{{ $title }}</option>
                            @endforeach
                        </select>
                        @error('province_id')
                            <div class="text-danger">{{ $message }}</div>
                        ...
                        <select name="district_id" id="district_id" class="form-control select2" required>
                            ...
                            @if(isset($districts))
                                @foreach($districts as $id => $title)
                                    <option value="{{ $id }}" {{ old('district_id', $isEdit ? $model->district_id : '') == $id ? 'selected' : '' }}>{{ $title }}</option>
                                @endforeach
                            @endif
                        </select>
                        @error('district_id')
...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            // Trigger change if old province_id exists (for edit mode)
            @if($isEdit && $model->district)
                $('#province_id').trigger('change');
                setTimeout(function() {
                    $('#district_id').val('{{ $model->district_id }}');
                }, 500);
            @endif
