...
@section('content')
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="card-title mb-0">{{ $title ?? 'Edit Union Council'}}</h5>
            <a href="{{ route('app.union-councils.index') }}" class="btn btn-secondary">Back to List</a>
        </div>

        <div class="card-body">
            <form action="{{ route('app.union-councils.update', encryptId($unionCouncil->id)) }}" method="POST" id="editUnionCouncilForm">
                @csrf
                @method('PUT')
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="title" class="form-control" placeholder="Enter Union Council Title"
                            value="{{ old('title', $unionCouncil->title) }}" required>
                        @error('title')
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="province_id" id="province_id" class="form-control select2" required>
                            ...
                            @foreach($provinces as $id => $title)
                                <option value="{{ $id }}" {{ (old('province_id', $unionCouncil->tehsil->district->province_id ?? '') == $id) ? 'selected' : '' }}>{{ $title }}</option>
                            @endforeach
                        </select>
                        @error('province_id')
                            <div class="text-danger">{{ $message }}</div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="district_id" id="district_id" class="form-control select2" required>
                            ...
                            @foreach($districts as $id => $title)
                                <option value="{{ $id }}" {{ (old('district_id', $unionCouncil->tehsil->district_id ?? '') == $id) ? 'selected' : '' }}>{{ $title }}</option>
                            @endforeach
                        </select>
                        @error('district_id')
                            <div class="text-danger">{{ $message }}</div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="tehsil_id" id="tehsil_id" class="form-control select2" required>
                            ...
                            @foreach($tehsils as $id => $title)
                                <option value="{{ $id }}" {{ (old('tehsil_id', $unionCouncil->tehsil_id) == $id) ? 'selected' : '' }}>{{ $title }}</option>
                            @endforeach
                        </select>
                        @error('tehsil_id')
                            <div class="text-danger">{{ $message }}</div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="ur_title" class="form-control" placeholder="Enter Urdu Title"
                            value="{{ old('ur_title', $unionCouncil->ur_title) }}">
                        @error('ur_title')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" name="short_title" class="form-control" placeholder="Enter Short Title"
                            value="{{ old('short_title', $unionCouncil->short_title) }}">
                        @error('short_title')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="active" class="form-control" required>
                            ...
                            <option value="1" {{ (old('active', $unionCouncil->active) == '1') ? 'selected' : '' }}>Active</option>
                            <option value="0" {{ (old('active', $unionCouncil->active) == '0') ? 'selected' : '' }}>Inactive</option>
                        </select>
                        @error('active')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea name="description" class="form-control" rows="3" placeholder="Enter Description">{{ old('description', $unionCouncil->description) }}</textarea>
                        @error('description')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#province_id').on('change', function() {
                ...
                var currentDistrictId = '{{ old("district_id", $unionCouncil->tehsil->district_id ?? "") }}';
            ...
            $('#district_id').on('change', function() {
                ...
                var currentTehsilId = '{{ old("tehsil_id", $unionCouncil->tehsil_id) }}';
                $('#tehsil_id').empty().append('<option value="">Select Tehsil</option>');
                ...
                if(districtId) {
                    $.ajax({
                        ...
                        success: function(data) {
                            $.each(data, function(key, value) {
                                var selected = (key == currentTehsilId) ? 'selected' : '';
                                $('#tehsil_id').append('<option value="' + key + '" ' + selected + '>' + value + '</option>');
                            });
                        }
                    });
                }
